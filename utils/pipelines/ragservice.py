...
from utils.pipelines.prompt_engineering import initial_prompt, summarize_opinion_prompt
import os
...
class RAGService:
    ...
    @instrument_retriever(name="hybrid_search", input_getter=lambda q, **_:q, output_getter=lambda out: out)
    def hybrid_search(self, query: str, k: int = 8) -> List[Dict[str, Any]]:
        return self.retriever.retrieve_hybrid(query, top_k=k)
    @instrument_retriever(name="keyword_search", input_getter=lambda q, **_:q, output_getter=lambda out: out)
    def keyword_search(self, query: str, k: int = 8) -> List[Dict[str, Any]]:
        return self.retriever.retrieve_bm25(query, top_k=k)
    @instrument_retriever(name="semantic_search", input_getter=lambda q, **_:q, output_getter=lambda out: out)
    def keyword_search(self, query: str, k: int = 8) -> List[Dict[str, Any]]:
        return self.retriever.retrieve_semantic(query, top_k=k)
    
    @instrument_llm(name="generate_text", input_getter=lambda x, **_: x, output_getter=lambda y: y)
    ...
    def run_pipeline(self, query: str) -> Dict[str, Any]:
        ...
        prompt = self.llm_inference(prompt)
        print(prompt)
